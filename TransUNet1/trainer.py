...
def trainer_synapse(args, model, snapshot_path):
    ...
    from datasets.dataset_multimodal import Synapse_dataset, RandomGenerator
    ...
    trainloader = DataLoader(db_train, batch_size=batch_size, shuffle=True, num_workers=1, pin_memory=True,
                             worker_init_fn=worker_init_fn)  # batch一个是多少，迭代是数量总数/batch
    if args.n_gpu > 1:
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if max_valus == 4:
                label_batch = torch.where(label_batch == 4, 3, label_batch)
