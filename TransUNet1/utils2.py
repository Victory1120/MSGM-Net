...
def calculate_metric_percase(pred, gt):
    ...
    if pred.sum() > 0 and gt.sum() > 0:
        ...
        return dice, hd95  #recall, precision, iou
...
def val_single_volume1(image, label, net, classes, patch_size=[224, 224], test_save_path=None, case=None, z_spacing=1, device='cpu'):
    # print(image.shape)
...
def val_single_volume(image, label, net, classes, patch_size=[224, 224], test_save_path=None, case=None, z_spacing=1, device='cpu'):
...
def test_single_volume(image, label, net, classes, patch_size=[224, 224], test_save_path=None, case=None, z_spacing=1, device='cpu'):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    if len(image.shape) == 3:
        prediction = np.zeros_like(label)
        for ind in range(image.shape[0]):
...
def test_single_volume2(image1, image2, image3, image4, label, net, classes, patch_size=[224, 224], test_save_path=None, case=None,z_spacing=1, device="cpu"):
    ...
    # max_valus = np.max(unique_values)
    # if max_valus == 4:
    ...
    if test_save_path is not None:
        img_itk1 = sitk.GetImageFromArray(image1.astype(np.float32))
        img_itk2 = sitk.GetImageFromArray(image2.astype(np.float32))
        img_itk3 = sitk.GetImageFromArray(image3.astype(np.float32))
        img_itk4 = sitk.GetImageFromArray(image4.astype(np.float32))
        prd_itk = sitk.GetImageFromArray(prediction.astype(np.float32))
        lab_itk = sitk.GetImageFromArray(label.astype(np.float32))
        img_itk1.SetSpacing((1, 1, z_spacing))
        img_itk2.SetSpacing((1, 1, z_spacing))
        img_itk3.SetSpacing((1, 1, z_spacing))
        img_itk4.SetSpacing((1, 1, z_spacing))
        prd_itk.SetSpacing((1, 1, z_spacing))
        lab_itk.SetSpacing((1, 1, z_spacing))
        sitk.WriteImage(prd_itk, test_save_path + '/' + case + "_pred.nii.gz")
        sitk.WriteImage(img_itk1, test_save_path + '/' + case + "_img1.nii.gz")
        sitk.WriteImage(img_itk2, test_save_path + '/' + case + "_img2.nii.gz")
        sitk.WriteImage(img_itk3, test_save_path + '/' + case + "_img3.nii.gz")
        sitk.WriteImage(img_itk4, test_save_path + '/' + case + "_img4.nii.gz")
        sitk.WriteImage(lab_itk, test_save_path + '/' + case + "_gt.nii.gz")
    return metric_list
def test_single_volume_1(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1,
    ...
    if x != patch_size[0] or y != patch_size[1]:
        # 缩放图像符合网络输入
        image1 = zoom(image, (1, patch_size[0] / x, patch_size[1] / y), order=3)
    input = torch.from_numpy(image1).unsqueeze(0).float().cuda()
    net.eval()
    with torch.no_grad():
        out = torch.argmax(torch.softmax(net(input), dim=1), dim=1).squeeze(0)
        out = out.cpu().detach().numpy()
    ...
    for i in range(1, classes):
        metric_list.append(calculate_metric_percase(prediction == i, label == i))
    if test_save_path is not None:
        gray_img = image.transpose(1, 2, 0)
        gray_img = cv2.cvtColor(gray_img, cv2.COLOR_BGR2GRAY)
        img_itk = sitk.GetImageFromArray(gray_img.astype(np.float32))
        prd_itk = sitk.GetImageFromArray(prediction.astype(np.float32))
        lab_itk = sitk.GetImageFromArray(label.astype(np.float32))
        img_itk.SetSpacing((1, 1, z_spacing))
        prd_itk.SetSpacing((1, 1, z_spacing))
