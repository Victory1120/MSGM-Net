...
class PreActBottleneck(nn.Module):
    ...
    def __init__(self, cin, cout=None, cmid=None, stride=1):
        ...
        self.conv1 = conv1x1(cin, cmid, bias=False)  # 64,64,没加strid,所以strid = 1??
    ...
    def forward(self, x):
        ...
        # Residual branch
        residual = x  # （8,64, 55,55）(8,256,55,55)
...
class ResNetV2(nn.Module):
    ...
    def __init__(self, block_units, width_factor):
        super().__init__()
        width = int(64 * width_factor)  # width_factor = 1, width = 64
        self.width = width
    ...
    def forward(self, x):
        ...
        x = self.root(x)  # x = (8,3,224,224)--> x = (8, 64, 112, 112)
        features.append(x)  # features = (8, 64, 112, 112)
        x = nn.MaxPool2d(kernel_size=3, stride=2, padding=0)(x)  # x = (8, 64, 55, 55)
        for i in range(len(self.body)-1):
            ...
            if x.size()[2] != right_size:  # 56!=55,
                ...
                feat = x
            features.append(feat)  # (8,64, 112, 112) (8, 256, 56, 56) (8, 512, 28, 28) (8, 1024, 14, 14)
        x = self.body[-1](x)
        return x, features[::-1]
