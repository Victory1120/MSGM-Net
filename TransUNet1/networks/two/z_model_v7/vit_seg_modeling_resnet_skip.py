...
from z_model_v7.epsanet import EPSANet,EPSABlock
...
class resnet1(nn.Module):
    ...
    def __init__(self, cin, cout=None, stride=1):
        ...
        self.conv1 = Conv2d(cin, cin, kernel_size=1, stride=2)  # 64,64,没加strid,所以strid = 1??
...
class ResNetV2(nn.Module):
    ...
    def __init__(self, block_units, width_factor):  # block_units = [3,3,6,9]
        super().__init__()
        width = int(32 * width_factor)  # width_factor = 1, width = 32
        self.width = width

        self.root = nn.Sequential(
            OrderedDict([
            ('conv', Conv2d(3, width, kernel_size=3, stride=2, padding=1)),
        ...
        self.body = nn.Sequential(OrderedDict([  # ResNet1,ResNet2
            ('block1', nn.Sequential(OrderedDict(
                [('unit1', resnet1(cin=width, cout=width*2, stride=2))] +  # 32, 64, 32
                [(f'unit{i:d}', resnet2(cin=width*2, cout=width*2)) for i in range(2, block_units[0] + 1)],  # 3 layers
                ))),
            ('block2', nn.Sequential(OrderedDict(
                [('unit1', resnet1(cin=width*2, cout=width*4, stride=2))] +
                [(f'unit{i:d}', resnet2(cin=width*4, cout=width*4)) for i in range(2, block_units[1] + 1)],  # 3 layers
                ))),
            ('block3', nn.Sequential(OrderedDict(
                [('unit1', resnet1(cin=width*4, cout=width*8, stride=2))] +
                [(f'unit{i:d}', resnet2(cin=width*8, cout=width*8)) for i in range(2, block_units[2] + 1)],  # 6 layers
                ))),
            ('block4', nn.Sequential(OrderedDict(
                [('unit1', resnet1(cin=width * 8, cout=width * 16, stride=2))] +
                [(f'unit{i:d}', resnet2(cin=width * 16, cout=width * 16)) for i in
                 range(2, block_units[3] + 1)],  # 9 layers
            ))),
        ...
        self.SE = nn.Sequential(OrderedDict([
            ...
            ('SE3', Squeeze_Excite_Block(width * 8)),
        ]))
    ...
    def forward(self, x):
        ...
        x = self.root(x)  # x = (8,3,224,224)--> x = (8, 32, 112, 112)
        for i in range(len(self.body)-1):  # 0,1,2
            ...
            x = self.SE[i](x)
            feat = x
            features.append(feat)  # (8,64, 112, 112) (8, 128, 56, 56) (8, 256, 28, 28)
